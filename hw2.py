...
class SarcasmDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if text is None:
            raise KeyError("Expected key 'headline' in each jsonl row.")

        # label (handle common variants just in case)
        if "is sarcastic" in item:
            label = int(item["is sarcastic"])
        elif "is_sarcastic" in item:
            label = int(item["is_sarcastic"])
        elif "label" in item:
            label = int(item["label"])
        else:
            raise KeyError("Expected label key: 'is sarcastic' (or 'is_sarcastic' / 'label').")

        encoding = self.tokenizer.encode_plus(
            text,
            add_special_tokens=True,
            max_length=self.max_length,
            padding="max_length",     # pad on the right to max_length
            truncation=True,          # truncate on the right to max_length
            return_attention_mask=True,
            return_tensors="pt",
        )

        input_ids = encoding["input_ids"].flatten().to(dtype=torch.long)            # (max_length,)
        attention_mask = encoding["attention_mask"].flatten().to(dtype=torch.long)  # (max_length,)
        label_tensor = torch.tensor(label, dtype=torch.long).view(1)                # (1,)
...
class SarcasmBERT(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        cls_repr = last_hidden_state[:, 0, :]                         # (batch_size, hidden)
    ...
    **kwargs
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm(dataloader, desc=f"Epoch {epoch+1}/{epochs}"):
            ...
            labels = batch["label"].to(device).view(-1)  # (batch_size,)
